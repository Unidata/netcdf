...
At the moment, this document only applies to the parts of the netCDF-C library
that use libcurl. Currently this encompasses the following protocols:
* DAP2 access
* DAP4 access
* byte-range access

With some exceptions (e.g. see the section on
<a href="#auth_redir">redirection</a>)
The libcurl authorization mechanisms can be accessed in two ways

1. Inserting the username and password into the url, or
2. Accessing information from a so-called _rc_ file named
   `.dodsrc` or any of its aliases: `.daprc` or .ncrc.

As discussed <a href="#auth_redir">later</a>,
the latter also allows for the specification
of a _.netrc_ file for use by libcurl.
...
escaped. See the <a href="#auth_userpwdescape">password escaping</a> section to see
...
".dodsrc" or ".daprc" or ".ncrc". They are searched for in that
order and the first one found is used.

It is strongly suggested that you pick one of the names
...
3. Search the HOME directory (`$HOME`).  The HOME environment
...
The rc file format is a series of lines of one of these general forms:
* \<key\>=\<value\>
* [https://\<host\>]\<key\>=\<value\>
* [https://\<host:port\>]\<key\>=\<value\>
* [https://\<host\>/\<path]\<key\>=\<value\>
* [https://\<host:port\>/\<path]\<key\>=\<value\>

## URL Constrained RC File Entries

Each line of the rc file can begin with an optional URI enclosed
in square brackets.  The only parts of the URI that are
currently used are the host, the port, and the path.
For backward compatibility, these reduced formats are also accepted:
* [\<host:port\>]\<key\>=\<value\>
* [\<host\>]\<key\>=\<value\>

Note that in most cases, only the host + port is used because
libcurl's authorization grain is not any finer than host level.
However, when accessing certain cloud storage systems (i.e. Amazon S3),
the path may also be used.

Examples.
* [remotetest.unidata.ucar.edu]HTTP.VERBOSE=1
* [https://fake.ucar.edu:9090]HTTP.VERBOSE=0
* [https://s3.us-east-1.amazonaws.com/mybucket]HTTPVERBOSE=1
...
See the <a href="#auth_userpwdescape">password escaping</a> section
...
# <a name="auth_userpwdescape">Password Escaping</a> {#auth_userpwdescape}
...
it is assumed that all occurrences of `user:password` use URL style (i.e. \%XX)
escaping for at least the characters in the table below.

The minimum set of characters that must be escaped depends on
the location.  If the user+pwd is embedded in the URL, then
occurrences of '@' and ':' __must__ be escaped if they are part of the
...
# <a name="auth_redir">Redirection-Based Authentication></a> {#auth_redir}
...
`.dodsrc` file (or from the url) for all connections that request
...
to use the `.dodsrc` password always, but this introduces a
...
___strongly___ encouraged to create a `.netrc` file to hold the
...
putting the following line in your `.dodsrc` file.
...
<a href="auth_redir">redirection based authorization section</a>

It is best to consult the URS website to determine how to access it
programmatically. However, the following instructions may work.
...
It is best to consult the ESG website to determine how to access it
programmatically. However, the following instructions may work.
